...
- README demo animation showing the detect → associate → crop → downstream pipeline,
  generated by `tools/make_demo_gifs.py` from procedural geometry (no footage, no weights).
  The pairing it shows comes from the real association node, so the animation cannot
  advertise behaviour the code does not have.
- `tests/test_demo_assets.py` fails when the committed animation no longer matches what the
  generator would produce, so a node or rule change cannot silently leave the README stale.
- `Pillow` added to the `dev` extra, needed only to regenerate the animation.
